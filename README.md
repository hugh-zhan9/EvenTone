...
进入校准时，日常播放的整体音量、设备补偿和自动均衡继续生效；原先关闭的处理保持关闭。参考声独立发送到所选设备，使用已选补偿一次，不再经过普通处理。试听时请暂停其他播放，避免声音叠加影响比较。取消不保存草稿，也不重启正常运行的处理通路。

连接状态只在点击试听时检查，而且只检查本次试听的设备；进入、选择设备和保存时不要求两副设备同时在线。试听失败保留草稿，连接好后再次点击试听即可。系统默认输出变化时，日常处理加载新设备的补偿，校准参考设备仍保持最初选择。参考声只在内存生成，不使用麦克风，不改变系统默认输出和硬件音量。
